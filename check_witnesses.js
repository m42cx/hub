...
function notifyAdmin(message) {
    write(message);
    if (!conf.admin_email || !conf.from_email) { return write('cannot notify admin as admin_email or from_email are not defined'); }
    mail.sendmail({
        to: conf.admin_email,
...
storage.readLastMainChainIndex((last_mci) => {
    db.query(
        ...
        (rows) => {
            ...
            const arrMissingWitnesses = rows.map(row => row.address);
